...
- Attendance, Kids management, Profiles, Change password
- Plain-text passwords (simple, editable)
"""
...
DEFAULT_IMAGE = ""  # use empty to show emoji placeholder
...
# ---------------- Initialize files and starter data ----------------
def init_files_and_starter():
    ...
    if users.empty:
        ...
        atomic_save(USERS_CSV, default_users)

    # starter import of KidsT.xlsx if kids.csv empty and file exists
...
def save_kids_df(df: pd.DataFrame):
    atomic_save(KIDS_CSV, df)

def save_users_df(df: pd.DataFrame):
    atomic_save(USERS_CSV, df)

def save_att_df(df: pd.DataFrame):
    atomic_save(ATT_CSV, df)
...
def page_login():
    ...
    st.write("Choose role and sign in. Default credentials: admin / 123 | leader1 / 123")
...
def page_dashboard():
    ...
    st.markdown("---")
    st.subheader("Recent attendance (latest 20)")
    if att.empty:
...
def page_admin_tools():
    ...
    st.markdown("---")
    st.subheader("Change own password")
    users = load_csv(USERS_CSV)
...
# Dashboard (admin)
if "Dashboard" in tab_map and tab_map["Dashboard"].selected:
    page_dashboard()

# Kids
if "Kids" in tab_map and tab_map["Kids"].selected:
    page_kids()

# Attendance
if "Attendance" in tab_map and tab_map["Attendance"].selected:
    page_attendance()

# Programs
if "Programs" in tab_map and tab_map["Programs"].selected:
    page_programs()

# Profiles
if "Profiles" in tab_map and tab_map["Profiles"].selected:
    page_profiles()

# Import (Admin) - this tab simply reuses import area in kids_page, but keep shortcut
if "Import" in tab_map and tab_map["Import"].selected:
    page_kids()

# Export
if "Export" in tab_map and tab_map["Export"].selected:
    page_export()

# Admin Tools
if "Admin Tools" in tab_map and tab_map["Admin Tools"].selected:
    page_admin_tools()

# Account (change own password)
if "Account" in tab_map and tab_map["Account"].selected:
    st.header("Account")
...
# Logout
if "Logout" in tab_map and tab_map["Logout"].selected:
